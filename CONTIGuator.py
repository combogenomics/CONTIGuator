...
def WriteMap(ContigsMap,sContig,sRef,oCFs,bMoreOutputs,mylog):
    ...
    # Outputs
    sTCOut = 'ContigHits.tab'
    sTROut = 'ReferenceHits.tab'
    sAlterPC = 'PseudoContig.fsa'
    sAlterC = 'PseudoContig.crunch'
    sAlterT = 'PseudoContig.tab'
    ...
    oCFs.addACTFile('ContigHits.tab')
    oCFs.addACTFile('ReferenceHits.tab')
    oCFs.addACTFile('PseudoContig.fsa')
    oCFs.addACTFile('PseudoContig.crunch')
    oCFs.addACTFile('PseudoContig.tab')
    oCFs.setCrunchFile(sRef,sAlterC)
    oCFs.setTabFile(sRef,'PseudoContig.tab')
    ...
    fAlterT = open(sAlterT, 'w')
    fTCOut = open(sTCOut, 'w')
    fTROut = open(sTROut, 'w')
    ...
    fAlterT.write('ID   '+oCFs.refNames[sRef]+'\n')
    fTCOut.write('ID   '+oCFs.refNames[sRef]+'\n')
    fTROut.write('ID   '+oCFs.refNames[sRef]+'\n')
    PContig = ''
    ...
    for cMap in ContigsMap:
        ...
        # PseudContig.tab
        if cMap.strand == '+':
            fAlterT.write('FT   contig          '+str(cMap.start)+
                          '..'+str(cMap.end)+'\n')
        else:
            fAlterT.write('FT   contig          complement('+str(cMap.start)+
                          '..'+str(cMap.end)+')\n')
        fAlterT.write('FT                   /systematic_id="'+cMap.name+'"\n')
        fAlterT.write('FT                   /method="blast"\n')
        if cMap.overlap:
            fAlterT.write('FT                   /Overlapping="YES"\n')
            fAlterT.write('FT                   /colour="5"\n')
        else:
            fAlterT.write('FT                   /colour="4"\n')
        ...
        for cprof in oCFs.profiles:
            ...
            for hit in cprof.getHits():
                ...
                # Hits tab files
                if cprof.getStrand() == '+':
                    WriteHitTabEntry(fTCOut, str(cMap.start+hit.qstart),
                         str(cMap.start+hit.qend),
                         hit.strand, hit.name)
                else:
                    WriteHitTabEntry(fTCOut,
                         str(cMap.start + (cprof.length - hit.qend)),
                         str(cMap.start + (cprof.length - hit.qstart)),
                         hit.strand, hit.name)
                #
                WriteHitTabEntry(fTROut, str(hit.sstart), str(hit.send),
                         hit.strand, hit.name)
    ...
    fAlterPC.write('>PseudoContig_'+oCFs.refNames[sRef]+'\n')
...
def Mapper(sContig,sRef,oCFs,mylog):
    ...
    for cMap in ContigsMap:
        if bStart:
            ...
            continue
        previous = ContigsMap[ContigsMap.index(cMap) - 1]
        if cMap.start - previous.end > 100:
            slideMap =  cMap.start - previous.end - 100
            cMap.start -= slideMap
            cMap.end -= slideMap
            # Apply the slide change to the subsequent contigs
            for cOther in ContigsMap[ContigsMap.index(cMap)+1:]:
...
def PrimerProductTab(products,sOut):
    o = open(sOut, 'w')
    o.write('ID   PrimerOutput\n')
    
    for p in products:
        o.write('FT   primer          '+str(p.getLeftPrimer().start)+'..'+
                str(p.getRightPrimer().start)+'\n')
        o.write('FT                   /systematic_id="'+str(p.name)+'"\n')
        o.write('FT                   /method="Abacas/Primer3"\n')
        o.write('FT                   /colour="1"\n')
...
def CONTIGuator(options):
    ...
    for sRef in oCFs.references.keys():
        ...
        oCFs.setTabFile(sRef, sRefDir+'/'+oCFs.tab[sRef])
    ...
    if options.bPrimer:
        for sRef in oCFs.references.keys():
            ...
            PrimerProductTab(products,'PCRProducts.tab')
            # Move the result
            shutil.copy('PCRProducts.tab',sRefDir)
            oCFs.primers[sRef]=sRefDir+'/PCRProducts.tab'
